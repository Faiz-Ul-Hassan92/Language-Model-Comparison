...
def train_model(model, optimizer_name, epochs=30, batch_size=128):
    ...
    print(f"\nTraining RNN with {optimizer_name.upper()} optimizer...")
...
def plot_history(history, optimizer_name):
    ...
    ax1.set_ylabel('Loss')
    ax1.set_title(f'RNN + {optimizer_name.upper()} - Loss')
    ax1.legend()
    ax1.grid(True)
    ...
    ax2.set_title(f'RNN + {optimizer_name.upper()} - Accuracy')
    ax2.legend()
    ax2.grid(True)
...
for opt in optimizers:
    print(f"Rnn with {opt.upper()}")
    
    model = build_rnn_model(vocab_size, max_sequence_len)
